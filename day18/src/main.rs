...
fn print_list(list: &LinkedList<Token>) {
    for (i, t) in list.iter().enumerate() {
        println!("list[{:?}]: {:?}", i, t);
    }
}

fn reduce(list: &mut LinkedList<Token>) -> Result<(), Box<dyn Error>> {
    if list.len() < 3 {
        return Ok(());
    }
    print_list(list);
    loop {
        ...
        let c = list[list.len() - 1];
        let b = stack[list.len() - 2];
        let a = stack[list.len() - 3];
        match (a, b, c) {
